...
const ApplyTemplateToRoom: React.FC<ApplyTemplateProps> = ({
   templateId: propsTemplateId,
   roomId: propsRoomId,
   onClose,
}) => {
   ...
   useEffect(() => {
      const loadData = async () => {
         ...
         if (targetId) {
            ...
            const templateRes =
               await seatTemplateService.getTemplateById(targetId);
   ...
   const handleApply = async () => {
      if (!selectedTemplateId || !selectedRoomId)
         return toast.error("Vui lòng chọn đầy đủ thông tin");

      if (
         window.confirm(
            "Sau khi áp dụng, sơ đồ cũ của phòng này (nếu có) sẽ bị xóa. Tiếp tục?",
         )
      ) {
         try {
            await seatTemplateService.applyTemplateToRoom({
      ...
      <div className="w-full bg-[#1a1a1e] p-8 rounded-xl border border-[#393939] shadow-2xl space-y-6">
         <div className="flex justify-between items-center border-b border-[#393939] pb-4">
            ...
            {onClose && (
               <button
                  onClick={onClose}
                  className="text-[#797b7d] hover:text-white transition-colors"
               >
                  ✕
               </button>
            )}
         ...
         <div className="grid grid-cols-2 gap-6">
          ...
          >
            ...
            <div>
               <label className="block text-sm font-bold text-[#797b7d] mb-2">
                  1. Choose Room
               </label>
               <select
                  className="w-full p-2.5 bg-[#252529] border border-[#393939] text-white rounded-lg outline-none focus:ring-1 focus:ring-[#f84565] focus:border-[#f84565]"
