...
const MovieCard: React.FC<{ movie: Movie }> = ({ movie }) => {
   return (
      <Link
         to={`/movies/${movie.id}`}
         className="z-50 rounded-2xl overflow-hidden transition-all duration-300 cursor-pointer flex flex-col h-full hover:-translate-y-1 hover:shadow-[0_16px_32px_rgba(248,69,101,0.2)] no-underline text-inherit bg-[#1e2939]"
      >
