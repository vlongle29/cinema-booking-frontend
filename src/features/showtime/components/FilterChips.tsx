...
import type { City, Format } from "../types/showtime.types";
...
interface FilterChipsProps {
   ...
   loading: boolean;
   error: string | null;
   itemNameFormatter?: (name: string) => string; // Prop mới để định dạng tên item
   isEmpty: boolean;
}
   ...
   return (
      <div className="mb-4">
         ...
         {loading ? (
            ...
            <EmptyState
               title="Không có lựa chọn nào"
               description={`Không có ${label.toLowerCase()} nào khả dụng cho lựa chọn của bạn`}
            />
         ) : (
            <div className="flex gap-2 flex-wrap">
               {items.map((item) => {
                  ...
                  const displayedItemName = itemNameFormatter
                     ? itemNameFormatter(itemName)
                     : itemName;
