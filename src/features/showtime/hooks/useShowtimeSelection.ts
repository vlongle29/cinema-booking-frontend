...
const useShowtimeSelection = () => {
   ...
   useEffect(() => {
      const loadDates = async () => {
         ...
         try {
            ...
            const response: any =
               await showtimeService.getAvailableDate(movieId);
            ...
            setDatesApi({
               loading: false,
               error:
                  error.message || "Failed to load dates. Please try again.",
            });
         }
      };
   ...
   useEffect(() => {
      ...
      const loadCities = async () => {
         ...
         try {
            ...
            const response: any = await showtimeService.getAvailableCity(
               movieId,
               selectedDate,
            );
   ...
   useEffect(() => {
      ...
      const loadFormats = async () => {
         ...
         try {
            await delay(800); // Artificial delay
            const response: any = await showtimeService.getAvailableFormat(
               movieId,
               selectedDate,
               selectedCity,
            );
            ...
            const formattedFormats: Format[] = data.map((item) => ({
               ...
               badge: item.format.includes("IMAX")
                  ? "Premium"
                  : item.format.includes("4DX")
                    ? "Experience"
                    : "Standard",
            }));
            ...
            setFormatsApi({
               loading: false,
               error:
                  error.message || "Failed to load formats. Please try again.",
            });
         }
      };
   ...
   useEffect(() => {
      ...
      const loadTheaters = async () => {
         ...
         try {
            ...
            const data: any[] = response.data || [];
            const mappedTheaters: Theater[] = data.map((branch: any) => {
               const cityObj = cities.find((c) => c.id === selectedCity);

               return {
                  id: branch.branchId,
                  name: branch.branchName,
                  location: cityObj ? cityObj.name : "Địa điểm",
                  showtimes: (branch.showtimes || []).map((st: any) => {
                     ...
                     return {
                        id: st.showtimeId,
                        roomId: st.roomId,
                        roomName: st.roomName, // Lưu lại roomName để component Grouping
                        time: timeStr,
                        isPeak: isPeakHour,
            ...
            setTheatersApi({
               loading: false,
               error:
                  error.message || "Failed to load theaters. Please try again.",
            });
         }
      };
